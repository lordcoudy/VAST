#!/usr/bin/env python3
"""Exact-input Savant container runtime for publication ABI v3.
...
EXPECTED_COORDINATOR_PATH = "/usr/local/bin/vast_savant_checkpoint_runtime"
DATASET_BY_CODEC = {
...
class SavantPublicationRuntimeV3Error(NativePublicationPermanentErrorV3):
    """The exact Savant container boundary is invalid or changed."""
...
def _canonical(value: object) -> bytes:
    try:
        ...
        _fail("savant_container_image_inspect_payload_invalid")
...
def _fd_hash(fd: int) -> tuple[int, str]:
    try:
        ...
        _fail("savant_runtime_input_descriptor_unreadable")
...
def _project_path(root: Path, raw: Any) -> Path:
    if (
        ...
        _fail("savant_runtime_input_path_invalid")
    ...
    try:
        ...
        _fail("savant_runtime_input_path_escaped_project_root")
    if resolved != path:
        _fail("savant_runtime_input_path_alias_forbidden")
    return path
...
def _container_input_path(raw: Any) -> str:
    if (
        ...
        _fail("savant_container_input_path_invalid")
    pure = PurePosixPath(raw)
    try:
...
def _open_pin(
    ...
    *,
    ...
    if type(value) is not dict or set(value) != DESCRIPTOR_FIELDS:
        _fail("savant_runtime_file_descriptor_fields_drifted")
    ...
    if (
        ...
        _fail("savant_runtime_file_descriptor_identity_invalid")
    ...
    try:
        ...
        if fd >= 0:
            os.close(fd)
        _fail("savant_runtime_file_descriptor_pin_failed")
    return _Pin(
...
def _open_list(root: Path, role: str, value: Any) -> tuple[_Pin, ...]:
    if type(value) is not list or not value or len(value) > MAX_FILES:
        _fail(f"savant_runtime_{role}_descriptor_set_invalid")
    result: list[_Pin] = []
    try:
...
def _socket_binding(value: Any) -> dict[str, Any]:
    if type(value) is not dict or set(value) != SOCKET_FIELDS:
        _fail("savant_container_engine_socket_binding_invalid")
    path = _absolute_socket_path(
        value.get("path"), "savant_container_engine_socket_binding_invalid"
    )
    if any(
...
def _endpoint_binding(value: Any) -> dict[str, Any]:
    if type(value) is not dict or set(value) != ENDPOINT_SOCKET_FIELDS:
        _fail("savant_endpoint_socket_binding_invalid")
    host = _absolute_socket_path(
        value.get("host_path"), "savant_endpoint_socket_binding_invalid"
    )
...
def _require_socket(value: Mapping[str, Any], *, engine: bool) -> None:
    ...
    blocker = (
        "savant_container_engine_socket_identity_changed"
        if engine else "savant_endpoint_socket_identity_changed"
    )
    try:
...
def _validate_image(value: Any) -> dict[str, Any]:
    if type(value) is not dict or set(value) != IMAGE_FIELDS:
        _fail("savant_container_image_contract_fields_drifted")
    ...
    if (
        ...
        _fail("savant_container_image_contract_invalid")
    return dict(value)
...
def _validate_dataset(request: NativePublicationRequestV3, source_values: Any) -> None:
    ...
    if (
        ...
        _fail("savant_frozen_kpp_dataset_binding_invalid")
    ...
    if (
        ...
        _fail("savant_frozen_kpp_stream_binding_invalid")
    if type(source_values) is not list:
        _fail("savant_runtime_source_files_descriptor_set_invalid")
    source_hashes = {
...
def _validate_contract(request: NativePublicationRequestV3) -> _Contract:
    ...
    if (
        ...
        _fail("savant_runtime_topology_binding_invalid")
    if runtime.get("policy") not in POLICIES:
        _fail("savant_runtime_policy_binding_invalid")
    _validate_dataset(request, value.get("source_files"))
    if type(value.get("defer_full_resource_acceptance")) is not bool:
        _fail("savant_resource_acceptance_mode_invalid")
    if (
        type(value.get("start_lead_ms")) is not int
        or not 0 <= value["start_lead_ms"] <= 60_000
    ):
        _fail("savant_start_lead_contract_invalid")
    _positive(value.get("ready_timeout_s"), "savant_ready_timeout_contract_invalid")
    _positive(value.get("drain_timeout_s"), "savant_drain_timeout_contract_invalid")
    _positive(value.get("container_timeout_s"), "savant_container_timeout_contract_invalid")
    ...
    if (
        ...
        _fail("savant_runtime_scratch_root_invalid")
    try:
...
def _open_pins(request: NativePublicationRequestV3, contract: _Contract) -> _Pins:
    ...
    try:
        ...
        if (
            ...
            _fail("savant_runtime_descriptor_path_set_invalid")
...
def _require_pins_unchanged(pins: _Pins) -> None:
    for pin in pins.all:
        try:
            ...
            _fail("savant_runtime_input_path_identity_changed")
        if (
...
def _invoke_engine(
    ...
    try:
        ...
        raise NativePublicationTransientErrorV3(
            "savant_container_engine_temporarily_unavailable"
        ) from error
    ...
    if timed_out:
        raise NativePublicationTransientErrorV3(
            "savant_container_execution_timed_out"
        )
    return _Completed(
...
def _inspect_image(pins: _Pins, contract: _Contract) -> dict[str, Any]:
    ...
    if (
        ...
        _fail("savant_container_image_identity_changed")
    return value
...
def _copy_pin(pin: _Pin, input_root: Path) -> None:
    ...
    try:
        ...
        _fail("savant_runtime_input_materialization_failed")
    finally:
        if fd >= 0:
            os.close(fd)
    if (
...
def _mount(source: str, target: str, *, readonly: bool) -> tuple[str, str]:
    if "," in source or "," in target:
        _fail("savant_container_mount_path_invalid")
...
def _validate_status(
    ...
    try:
        ...
        _fail("savant_native_runtime_terminal_status_invalid")
    runtime = request.runtime_inputs
...
def run_checkpoint_savant_publication_runtime_v3(
    ...
    """Run one exact Savant image and map only declared evidence."""
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(
            ...
            if _is_link(scratch.lstat()):
                _fail("savant_runtime_scratch_custody_invalid")
...
__all__ = [
    "SavantPublicationRuntimeV3Error",
    "RUNTIME_INPUT_KEY",
    "RUNTIME_INPUT_KIND",
