#!/usr/bin/env python3
"""DeepStream publication ABI-v3 coordinator executed inside the pinned image.
...
class DeepStreamContainerRuntimeV3Error(RuntimeError):
    """A materialized input, SDK graph, or publication gate failed closed."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise DeepStreamContainerRuntimeV3Error(message)
...
def _registry_factories(codec: str) -> tuple[str, ...]:
    _require(codec in {"h264", "h265"}, "DeepStream registry codec is invalid")
...
def seed_deepstream_gstreamer_registries(
    ...
    *,
    ...
    try:
        ...
        raise DeepStreamContainerRuntimeV3Error(
            "DeepStream registry root is invalid"
        ) from exc
    _require(
        resolved_root == root
    ...
    _require(bool(specs), "DeepStream registry process set is empty")
    ...
    for spec in specs:
        ...
        _require(
            ...
            "DeepStream registry environment is not fail-closed",
        )
        ...
        _require(
            ...
            "DeepStream registry destination is invalid",
        )
        destinations.append(destination)
    _require(
        len(destinations) == len(set(destinations)),
        "DeepStream registry destinations are not process-private",
    )

    descriptor, seed_name = tempfile.mkstemp(
        prefix="vast-deepstream-gpu-seed-",
        suffix=".bin",
        dir=str(root),
    )
    os.close(descriptor)
    ...
    try:
        ...
        for factory in factories:
            ...
            _require(
                ...
                f"DeepStream required GStreamer factory is unavailable: {factory}",
            )
            ...
            _require(
                ...
                "DeepStream registry scanner capture exceeded its bound",
            )
        seed_payload = _bounded_plain_payload(
            seed,
            limit=MAX_GSTREAMER_REGISTRY_BYTES,
            label="DeepStream GPU registry seed",
        )
        _require(seed_payload, "DeepStream GPU registry seed is empty")
        ...
        for destination in destinations:
            ...
            _require(
                ...
                "DeepStream registry copy digest drifted",
            )
        return {
            "schema_version": 3,
            "artifact_kind": "vast_deepstream_gstreamer_registry_seed_audit_v3",
        ...
        raise DeepStreamContainerRuntimeV3Error(
            "DeepStream GPU registry materialization failed"
        ) from exc
    finally:
...
@contextmanager
def _capture_deepstream_native_stdio(output_dir: Path):
...
def validate_deepstream_native_stdio(
    stdout_path: Path,
    stderr_path: Path,
    *,
    ...
    _require(
        ...
        "DeepStream native stdio worker count is invalid",
    )
    stdout = _bounded_plain_payload(
        stdout_path,
        limit=MAX_NATIVE_STDIO_BYTES,
        label="DeepStream native stdout",
    )
    stderr = _bounded_plain_payload(
        stderr_path,
        limit=MAX_NATIVE_STDIO_BYTES,
        label="DeepStream native stderr",
    )
    try:
        lines = [line for line in stdout.decode("ascii").splitlines() if line]
    except UnicodeDecodeError as exc:
        raise DeepStreamContainerRuntimeV3Error(
            "DeepStream native stdout contract is invalid"
        ) from exc
    _require(
        lines == [NVSTREAMMUX_EOS_LINE] * expected_worker_count,
        "DeepStream native stdout contract is invalid",
    )
    _require(not stderr, "DeepStream native stderr contract is not empty")
    return {
        "schema_version": 3,
        "artifact_kind": "vast_deepstream_native_stdio_audit_v3",
...
def _load_structured(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise DeepStreamContainerRuntimeV3Error(f"{label} is unreadable") from exc
...
def _validate_socket_path(value: Any, label: str) -> str:
    ...
    try:
        ...
        raise DeepStreamContainerRuntimeV3Error(f"{label} socket is missing") from exc
...
def validate_adapter_materialization(
    *,
    ...
    config = validate_adapter_config(_load_structured(adapter_config_path, "DeepStream adapter config"))
    _require(set(config["branches"]) == set(BRANCHES), "DeepStream adapter config lacks the frozen four branches")
    _require(
        Path(config["preprocessing_manifest_path"]) == analytics_model_manifest_path,
        "DeepStream preprocessing manifest path differs from the pinned manifest",
    )
    ...
    try:
        ...
        raise DeepStreamContainerRuntimeV3Error(
            "DeepStream capability bindings are missing"
        ) from exc
    ...
    _require(referenced_models == model_paths, "DeepStream model binding set has unreferenced material")
    return config


def _frame_transfer_bytes(dataset: Mapping[str, Any], stream_id: int) -> int:
    streams = dataset.get("streams")
    _require(isinstance(streams, list), "DeepStream dataset streams are missing")
    selected = next(
        (
            value
    ...
    _require(isinstance(selected, Mapping), f"DeepStream dataset lacks stream {stream_id}")
    width = int(selected.get("width", 0) or 0)
    height = int(selected.get("height", 0) or 0)
    _require(width > 0 and height > 0, f"DeepStream stream {stream_id} dimensions are invalid")
...
def write_native_stage_resource_events(
    ...
    *,
    ...
    _require(bool(frame_event_rows), "DeepStream native frame-event set is empty")
    rows: list[dict[str, Any]] = []
    for event in frame_event_rows:
        start = float(event["stage_start_timestamp_ms"])
        end = float(event["stage_end_timestamp_ms"])
        _require(math.isfinite(start) and math.isfinite(end) and start <= end, "DeepStream native stage interval is invalid")
        duration = end - start
        resource = str(event["resource"]).lower()
        _require(resource in {"cpu", "gpu", "nvdec"}, "DeepStream native stage resource is invalid")
    ...
    try:
        ...
        raise DeepStreamContainerRuntimeV3Error(
            "DeepStream resource_events.csv already exists"
        ) from exc
...
def _write_json(path: Path, value: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        raise DeepStreamContainerRuntimeV3Error(
            f"DeepStream runtime evidence already exists: {path.name}"
        ) from exc
...
def execute_publication_arm(args: argparse.Namespace) -> dict[str, Any]:
    """Execute one genuine DeepStream SDK arm and return the terminal ABI object."""

    import yaml
    import pandas as pd

    from benchmark_contract import INGRESS_LEDGER_COLUMNS, RESET_EVIDENCE_COLUMNS
    from checkpoint_deepstream_launcher import (
        build_deepstream_source_specs,
        build_deepstream_worker_specs,
    )
    from checkpoint_deepstream_runtime import build_deepstream_runtime_plan
    ...
    _require(os.name == "posix", "DeepStream publication coordinator requires POSIX")
    _require(args.output_dir == OUTPUT_ROOT, "DeepStream output directory differs from ABI-v3")
    _require(args.topology_kind == SCENARIO_TOPOLOGY[args.scenario], "DeepStream scenario/topology binding drifted")
    _require(args.policy in POLICIES, "DeepStream policy is outside the frozen matrix")
    _require(math.isfinite(args.deadline_ms) and args.deadline_ms > 0, "DeepStream deadline is invalid")
    _require(args.duration == 180, "DeepStream publication measurement duration must equal 180 seconds")
    _require(args.ready_timeout > 0 and args.drain_timeout > 0, "DeepStream lifecycle timeout is invalid")
    _require(0 <= args.start_lead_ms <= 60_000, "DeepStream start lead is invalid")
    _require(args.output_dir.is_dir() and not args.output_dir.is_symlink(), "DeepStream output mount is invalid")
    _require(not any(args.output_dir.iterdir()), "DeepStream output mount must be empty for each arm")
    ...
    _require((args.policy == "static_hybrid") == (static_path is not None), "DeepStream static-hybrid map binding drifted")
    ...
    physical_plan = build_deepstream_runtime_plan(
        config=config,
        datasets=datasets,
    ...
    _require(
        ...
        "DeepStream physical/publication plan identity drifted",
    )
    _require(
        ...
        "DeepStream frozen cohort window drifted",
    )
    expected_source_hashes = {
        str(source["source_sha256"]) for source in physical_plan["sources"]
    }
    _require(set(source_bindings) == expected_source_hashes, "DeepStream KPP source binding set drifted")
    ...
    _require(SOURCE_EXECUTABLE.is_file(), "DeepStream source coordinator executable is missing")
    runtime_output = args.output_dir / "native_runtime"
    runtime_output.mkdir(mode=0o700)
    worker_specs = build_deepstream_worker_specs(
        physical_plan,
        run_id=args.run_id,
        arm_id=args.arm_id,
    ...
    source_specs = build_deepstream_source_specs(
        physical_plan,
    ...
    _require(len(worker_specs) == expected_workers and len(source_specs) == 6, "DeepStream physical process cardinality drifted")
    registry_audit = seed_deepstream_gstreamer_registries(
        worker_specs,
        source_specs,
        codec=args.codec,
    )
    ...
    with topology_path.open("x", newline="", encoding="utf-8") as output:
        ...
        with _capture_deepstream_native_stdio(runtime_output) as native_stdio_paths:
    ...
    _write_json(
        runtime_output / "deepstream_runtime_manifest.json",
        {
            "schema_version": 3,
            "artifact_kind": "vast_deepstream_runtime_manifest_v3",
            "run_id": args.run_id,
            "arm_id": args.arm_id,
...
def terminal_status(
    ...
    *,
    ...
    _require(
        ...
        "DeepStream terminal status cannot promote an incomplete arm",
    )
    try:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one exact-input DeepStream publication ABI-v3 arm"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"deepstream publication runtime blocked: {exc}", file=sys.stderr)
        return 2
    print(
        json.dumps(
            status,
