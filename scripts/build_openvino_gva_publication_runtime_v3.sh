...
image_ref="${VAST_OPENVINO_GVA_RUNTIME_IMAGE:-vast/openvino-gva-publication-runtime-v3:materialized}"
first_ref="${image_ref}-determinism-a"
second_ref="${image_ref}-determinism-b"
source_date_epoch="0"

cd "$project_root"
source_allowlist="deploy/openvino_gva/publication/runtime-source-allowlist.txt"
dependency_allowlist="deploy/gstreamer_custom/publication/runtime-dependency-allowlist.txt"
build_context_allowlist="deploy/openvino_gva/publication/runtime-build-context-allowlist.txt"
/usr/bin/python3 \
...
[[ "$observed_base_id" == "$expected_base_id" ]] || {
  echo "pinned OpenVINO GVA base image ID drifted" >&2
  exit 2
}
...
compute_source_allowlist_sha256() {
  local root="$1"
  (cd "$root" && sha256sum "$source_allowlist" | cut -d' ' -f1)
}
...
runtime_source_allowlist_sha256="$(compute_source_allowlist_sha256 "$project_root")"
...
cleanup_build_context() {
  if [[ "$(dirname -- "$build_context")" != /tmp \
     || "$(basename -- "$build_context")" != vast-openvino-gva-publication-v3.* ]]; then
    echo "refusing unsafe OpenVINO GVA build-context cleanup" >&2
    return 2
  fi
  rm -rf -- "$build_context"
}
...
/usr/bin/python3 -B \
  "$build_context/deploy/openvino_gva/publication/validate_runtime_source_closure_v3.py" \
...
if [[ "$(compute_runtime_source_sha256 "$build_context")" != "$runtime_source_sha256" \
   ...
   || "$(compute_source_allowlist_sha256 "$build_context")" != "$runtime_source_allowlist_sha256" \
   || "$(compute_build_context_inputs_sha256 "$build_context")" != "$build_context_inputs_sha256" ]]; then
  echo "OpenVINO GVA exact build-context identity drifted" >&2
  exit 2
fi
...
build_one() {
  ...
  DOCKER_BUILDKIT=1 SOURCE_DATE_EPOCH="$source_date_epoch" docker build \
    ...
    --build-arg "VAST_RUNTIME_SOURCE_ALLOWLIST_SHA256=$runtime_source_allowlist_sha256" \
...
/usr/bin/python3 -B deploy/openvino_gva/publication/validate_runtime_source_closure_v3.py \
  --project-root "$project_root" \
  --manifest "$project_root/$source_allowlist"
if [[ "$(compute_runtime_source_sha256 "$project_root")" != "$runtime_source_sha256" \
   ...
   || "$(compute_source_allowlist_sha256 "$project_root")" != "$runtime_source_allowlist_sha256" \
   || "$(compute_build_context_inputs_sha256 "$project_root")" != "$build_context_inputs_sha256" ]]; then
  echo "OpenVINO GVA workspace inputs changed during build" >&2
  exit 2
fi
if [[ "$(compute_runtime_source_sha256 "$build_context")" != "$runtime_source_sha256" \
   || "$(compute_native_source_sha256 "$build_context")" != "$native_probe_source_sha256" \
   ...
   || "$(compute_source_allowlist_sha256 "$build_context")" != "$runtime_source_allowlist_sha256" \
   || "$(compute_build_context_inputs_sha256 "$build_context")" != "$build_context_inputs_sha256" ]]; then
  echo "OpenVINO GVA exact build context changed during build" >&2
  exit 2
fi
...
ready="$(docker image inspect "$first_id" --format '{{index .Config.Labels "org.vast.publication-ready"}}')"
...
allowlist_label="$(docker image inspect "$first_id" --format '{{index .Config.Labels "org.vast.runtime-source-allowlist-sha256"}}')"
...
[[ "$entrypoint" == '["/usr/local/bin/vast_openvino_gva_publication_runtime_v3"]' \
   ...
   && "$allowlist_label" == "$runtime_source_allowlist_sha256" \
   && "$created" == '1970-01-01T00:00:00Z' ]] || {
  echo "materialized OpenVINO GVA image projection is invalid" >&2
  exit 2
}
