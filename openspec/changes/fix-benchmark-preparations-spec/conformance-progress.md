...
The 2026-09-23 parity acceptance and 2,515-test ext4 suite passed on their then-current bytes but are invalidated by the 2026-09-24 publication runtime registry repair. All four packaged runtime images have since been rebuilt and refrozen from unchanged native/worker receipts; stock packaged checks and the network-isolated OpenVINO GPU decoder smoke pass. Exact patch-bound physical parity is running under one owner, while the final suite and qualification inputs still require renewal; see `implementation-validation.md`. The 43-test checkpoint runtime suite and 11-test codec/refresh suite pass on the repair, but they do not substitute for physical renewal.
