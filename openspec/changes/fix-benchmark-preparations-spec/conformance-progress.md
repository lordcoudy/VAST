...
This public record maps every requirement and scenario in the active change to source and evidence as of 2026-09-24. It is not a readiness receipt, CI result, qualification result, Q4 result, capacity confirmation, or authorization to start the full benchmark. No full-matrix arm has been launched by this change.

**Evidence terms:** **current local result** is an executed test or package check recorded for this change. **Current local fixture result** is executed test coverage that does not prove live qualification, Q4 or capacity. **Physical evidence pending** requires a real accepted operation or its current receipts. The final ext4 suite and focused repair checks are local, non-authorizing evidence; the collector's durable nine-test record and private paths are not published here.
...
- **Packaged source changes — historical Sep23 result, invalidated for current preparation.** All three native probes rebuilt with matching A/B receipts; two dependent workers and all four runtimes rebuilt and passed stock receipt/patch capture. Twelve packaged historical/native checks and ten protocol/bridge/sidecar/worker checks passed. The renewed parity service exited 0 on 480 physical executions in 32 groups of 15. The stock acceptance loader, 3,535 descriptor references and all 480 input/output tensor hashes passed independent verification against A244. Its acceptance identity is `f089b711…`. The final ext4 suite passed on the current source and fixture bytes: 2,515 tests, zero failures, 88 reviewed skips, unchanged source/mirror/fixture/recovered manifests, and 2,272 independently rehashed current files. Its passed result admitted the Sep23 qualification input chain, but does not establish current qualification or Q4 completion.
- **Deterministic planning replay — current local fixture result; accepted-identity replay pending.** The final Sep24 ext4 suite ran `tests/test_publication_matrix.py::PublicationMatrixTests.test_full_matrix_is_deterministic_complete_and_paired`, `tests/test_full_publication_entrypoint.py::OfflinePublicationPlanTests.test_real_plan_factory_is_offline_read_only_and_needs_no_run_root`, and `tests/test_publication_article_statistics_v1.py::PublicationArticleStatisticsV1Tests.test_twenty_sealed_arms_reproduce_exact_preregistered_paired_inference`; all three passed. No replay from current accepted Q4 preparation identities has run.

The 2026-09-23 parity acceptance and 2,515-test ext4 suite passed on their then-current bytes but are invalidated by the 2026-09-24 publication runtime registry repair. All four packaged runtime images have since been rebuilt and refrozen from unchanged native/worker receipts; stock packaged checks and the network-isolated OpenVINO GPU decoder smoke pass. New patch-bound physical parity completed under one owner: 480 executions, 32 groups, original exit 0, and an independent stock-loader/descriptor/tensor audit against A244 also exited 0. Host fragment pins have been renewed and focused tests pass; the final suite passed 2,516 tests with 88 reviewed skips and 2,286 independently rehashed current files; Sep24 qualification inputs and the gated diagnostic have since passed; a distinct 32-cell pilot is running. See `implementation-validation.md`. The 43-test checkpoint runtime suite and 11-test codec/refresh suite pass on the repair, but they do not substitute for physical renewal.

## 6. Qualification and Q4 are complete before launch readiness — unverified physical stages

Current local fixture result; physical stages pending: the final Sep24 ext4 suite ran `tests/test_publication_policy_qualification_pilot_executor_v2.py` (27 passed, two live DrvFS-only cases skipped on ext4), `tests/test_publication_policy_qualification_execution_closure_v1.py` (13 passed), `tests/test_publication_q4_authority_plan_pipeline_v1.py` (13 passed), `tests/test_backend_q4_two_phase_source_registry_v1.py` (11 passed), and `tests/test_backend_q4_two_phase_executor_v1.py` (38 passed, including one expected CLI rejection printed before its `ok`). These exercise validators, receipts, fail-closed behavior, qualification construction, Q4 phase chains, and 560/280 fixture cardinalities; they do not establish fresh physical qualification or Q4 acceptance.
...
- **Fresh qualification and Q4 succeed — in progress.** The fresh input/preprocessing chain, eight-worker guardian, 32 runtime bundles/four assets, and bounded Savant diagnostic passed their original checks. That 32-cell pilot (InvocationID `920b5cfea7474bcdbebb44434459bd3d`) failed after 16 cells at the first OpenVINO GVA cell. Its guardian subsequently completed an authenticated clean nonpublication stop. The partial cells and Sep23 source/image/qualification chain are historical. A distinct Sep24 transaction/preprocessing/guardian/materialization chain and one-container Savant diagnostic have now passed independent physical checks. Its new pilot (InvocationID `d6a86cf4cee04a6a9bce3411f4f4a546`) has an `in_progress` checkpoint; all 32 accepted cells, guardian closure, promotion and Q4 remain pending. No historical cells count toward it.
- **A prerequisite is missing or fails — current physical failure evidence and existing validator coverage.** The named closure and Q4 modules contain rejection cases for missing, stale, partial, tampered, and cross-bound inputs; the retained partial pilot and supported resume record real prerequisite failure, but no accepted completion.

## 7. Cloud admission uses an actual dated guarantee — unverified

Current local fixture result; operational admission pending: the final Sep24 ext4 suite ran all five `tests/test_full_publication_seafile_capacity_binding.py::FullPublicationSeafileCapacityBindingTests` cases successfully. They cover lower-bound attestation, destination binding, rotation/tamper rejection, and legacy-capacity rejection; no current dated physical admission or live readback has passed.

- **Confirmation missing — current local fixture result; operationally unverified.** The operator supplied a 600-GiB current guaranteed-free-capacity statement for the existing Seafile destination on 2026-09-23. The Q4 sizing-based attestation and live destination check remain pending.
- **Sizing or readback exceeds the guarantee — current local fixture result; operationally unverified.** There are no accepted Q4 sizing records or successful live upload/readback evidence.
...
- **Altered matrix or malformed command — current local result for drift guards; operational replay pending.** The final Sep24 ext4 suite ran `tests/test_full_publication_entrypoint.py::OfflinePublicationPlanTests.test_valid_alternate_2800_pair_matrix_is_permanent_78`, `test_cli_expected_pin_cannot_authorize_an_alternate_matrix`, and `ProductionEntrypointTests.test_plan_preflight_and_run_reject_matrix_identity_drift_before_execution`; all three passed. The runbook's Bash and parser checks also passed, but no command using the future accepted Q4 identities has run.
