#!/usr/bin/env python3
"""Validate the exact DeepStream ABI-v3 source and dependency closures."""
...
RUNTIME_FIXED_COPY_SOURCES = {
    DEPENDENCY_MANIFEST_RELATIVE_PATH,
    MANIFEST_RELATIVE_PATH,
    "deploy/deepstream/checkpoint/vast_deepstream_checkpoint_runtime",
    "deploy/deepstream/checkpoint/vast_deepstream_publication_runtime_v3",
}
NATIVE_ENTRY_SOURCE = (
    "deploy/native_gst_probe/checkpoint_source_coordinator.cpp"
)
DEEPSTREAM_NATIVE_ENTRY_SOURCE = (
...
_DEEPSTREAM_NATIVE_PATH = re.compile(
    r"deploy/deepstream/checkpoint/[a-z][a-z0-9_]*\.(?:cpp|h)"
)
...
def _validate_native_group(
    *,
    ...
    _require(
        all(path_pattern.fullmatch(value) for value in declared),
        f"native source declaration is outside {source_root}",
    )
    for value in declared:
        _require_regular_source(root, value)

    native_root = root.joinpath(*PurePosixPath(source_root).parts)
    _require(
        native_root.is_dir() and not native_root.is_symlink(),
        f"native source directory is invalid: {source_root}",
    )
    available = {
        source.name: source
        for source in native_root.iterdir()
        if source.is_file() and not source.is_symlink()
        and path_pattern.fullmatch(
            f"{source_root}/{source.name}"
        )
    }
    ...
    expected = {f"{source_root}/{name}" for name in reachable}
    observed = set(declared)
    missing = sorted(expected - observed)
    extra = sorted(observed - expected)
...
def validate_dependency_source_closure(
    *, project_root: Path, manifest_path: Path,
    ...
    wheel_entries = tuple(wheels.iterdir())
    _require(
        all(
            path.is_file()
    ...
    _require(
        observed_wheels == declared_wheels,
        "runtime wheel dependency set drifted",
    )
    return declared
...
def validate_runtime_source_closure(
    *, project_root: Path, manifest_path: Path,
    ...
    _require(raw and raw.endswith(b"\n") and b"\r" not in raw,
             "source allowlist must be LF-terminated")
    declared = tuple(text.splitlines())
    _require(
        declared == tuple(sorted(set(declared))) and bool(declared),
    ...
    deepstream_native_declared = tuple(
        value for value in declared if _DEEPSTREAM_NATIVE_PATH.fullmatch(value)
    )
    classified = (
        classified_fixed
        | set(python_declared)
        | set(native_declared)
        | set(deepstream_native_declared)
    )
    _require(
    ...
    native_sources = _validate_native_group(
        project_root=root,
        declared_paths=native_declared,
        path_pattern=_NATIVE_PATH,
        source_root="deploy/native_gst_probe",
        entry_source=NATIVE_ENTRY_SOURCE,
    )
    deepstream_native_sources = _validate_native_group(
        project_root=root,
        declared_paths=deepstream_native_declared,
        path_pattern=_DEEPSTREAM_NATIVE_PATH,
        source_root="deploy/deepstream/checkpoint",
        entry_source=DEEPSTREAM_NATIVE_ENTRY_SOURCE,
    )
    ...
    return {
        ...
        "deepstream_native_sources": deepstream_native_sources,
