...
import checkpoint_savant_publication_launcher_v3 as launcher  # noqa: E402
from checkpoint_savant_publication_runtime_v3 import (  # noqa: E402
...
EVIDENCE_NAME = "savant-native-evidence.json"
FILE_ROLES = (
    "container_engine",
    "experiments_config",
    "datasets_config",
...
REPO_DIGEST = "vast/savant-checkpoint@sha256:" + "b" * 64
IMAGE_LABELS = {
...
@unittest.skipUnless(os.name == "posix", "Savant container custody is Linux-only")
class SavantPublicationRuntimeV3Tests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        runtime_contract = {
            "schema_version": 3,
            "artifact_kind": "vast_savant_publication_runtime_inputs_v3",
            "files": files,
            "source_files": [source],
            "model_files": [model],
            ...
            "container_image": {
                ...
                "coordinator_path": "/usr/local/bin/vast_savant_checkpoint_runtime",
        ...
        self.request = NativePublicationRequestV3(
            ...
            runtime_inputs={
                ...
                "dataset": {
                    ...
                    "savant_publication_runtime_v3": runtime_contract,
                },
                "streams": 6,
                "duration_s": 180,
                "repeat_index": 0,
    ...
    def test_exact_engine_image_inputs_sockets_and_evidence_are_bound(self) -> None:
        ...
        def invoke(
            ...
            self.assertNotIn("vast/savant-checkpoint:latest", argv)
            output_mount = next(
                argv[index + 1]
            ...
            (runtime_output / EVIDENCE_NAME).write_bytes(b'{"savant":true}\n')
            status = {
                "schema_version": 3,
                "artifact_kind": "vast_savant_publication_terminal_status_v3",
        ...
        with mock.patch(
            "checkpoint_savant_publication_runtime_v3._invoke_engine",
            side_effect=invoke,
        ):
            outcome = run_checkpoint_savant_publication_runtime_v3(self.request)
    ...
    def test_image_inspect_identity_drift_is_rejected_before_run(self) -> None:
        ...
        with mock.patch(
            ...
            SavantPublicationRuntimeV3Error,
            "savant_container_image_identity_changed",
        ):
    ...
    def test_transient_input_path_replacement_is_rejected_before_container_run(self) -> None:
        ...
        module = __import__("checkpoint_savant_publication_runtime_v3")
        ...
        with mock.patch(
            ...
            "checkpoint_savant_publication_runtime_v3._require_pins_unchanged",
            side_effect=replace_then_check,
        ), self.assertRaisesRegex(
            SavantPublicationRuntimeV3Error,
            "savant_runtime_input_path_identity_changed",
        ):
            run_checkpoint_savant_publication_runtime_v3(self.request)
        self.assertFalse((self.output / EVIDENCE_NAME).exists())
    ...
    def test_contract_schema_dataset_and_evidence_mapping_are_closed(self) -> None:
        ...
        mutable_image["container_image"]["image_id"] = "vast/savant:latest"
        ...
        for index, contract in enumerate(mutations):
            with self.subTest(index=index):
                ...
                dataset["savant_publication_runtime_v3"] = contract
                runtime_inputs["dataset"] = dataset
                request = NativePublicationRequestV3(
                    **{**self.request.__dict__, "runtime_inputs": runtime_inputs}
                )
                with self.assertRaises(SavantPublicationRuntimeV3Error):
                    run_checkpoint_savant_publication_runtime_v3(request)
